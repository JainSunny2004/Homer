  ...
  useEffect(() => {
    ...
    for (let i = 0; i < devices.length; i++) {
      for (let j = i + 1; j < devices.length; j++) {
        ...
        // ✅ FIX: Skip if either device has no valid location
        const aValid = locA.latitude !== 0 || locA.longitude !== 0;
        const bValid = locB.latitude !== 0 || locB.longitude !== 0;
        if (!aValid || !bValid) continue;

        // ✅ FIX: Skip if either device has no signal
        if (locA.locationSource === "NONE" || locB.locationSource === "NONE")
          continue;
        ...
        if (dist <= coMovementThresholdMeters) {
          if (!existing) {
            ...
            !existing.alerted &&
            now - existing.startTime >= durationMs
          ) {
    ...
    tracker.forEach((v, k) => {
      if (!deviceSet.has(v.deviceA) || !deviceSet.has(v.deviceB))
        tracker.delete(k);
    });
  }, [
    locations,
